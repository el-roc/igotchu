  ...
  return (
    <section className="board">
      {this.state.posts.map((item, i) => {
        return <a href="/postedItem/:id"><ItemViewNewsfeed className="cell" key={(i).toString()} itemName={item.itemName} imageURL={"https://via.placeholder.com/200"} email={this.email} itemPrice={item.itemPrice}/>
        </a>;
      })}
    </section>
  );
